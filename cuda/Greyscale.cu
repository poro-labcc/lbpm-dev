...
__global__ void dvc_ScaLBL_D3Q19_AAodd_Greyscale(
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (n < finish) {
            ...
            nr2 = neighborList[n +
                               Np]; // neighbor 1 ( < 10Np => even part of dist)
...
__global__ void dvc_ScaLBL_D3Q19_AAeven_Greyscale_IMRT(
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (n < finish) {
            ...
            pressure = 0.5 / porosity *
                       (pressure - 0.5 * Den * u_mag * u_mag / porosity);
            ...
            m1 =
                m1 + rlx_setA *
                         ((-30 * Den +
                          ...
                          m1);
            m2 = m2 + rlx_setA * ((12 * Den -
                                   5.5 * Den * (ux * ux + uy * uy + uz * uz) /
                                       porosity -
            ...
            m10 =
                m10 +
                rlx_setA * (-0.5 * Den *
                                ((2 * ux * ux - uy * uy - uz * uz) / porosity) -
                            m10);
            m11 =
                m11 + rlx_setA * ((Den * (uy * uy - uz * uz) / porosity) - m11);
            m12 =
                m12 + rlx_setA *
                          (-0.5 * (Den * (uy * uy - uz * uz) / porosity) - m12);
            m13 = m13 + rlx_setA * ((Den * ux * uy / porosity) - m13);
            m14 = m14 + rlx_setA * ((Den * uy * uz / porosity) - m14);
            m15 = m15 + rlx_setA * ((Den * ux * uz / porosity) - m15);
            m16 = m16 + rlx_setB * (-m16);
            ...
            fq = mrt_V1 * Den + mrt_V9 * m1 + mrt_V10 * m2 + 0.1 * (jx + jy) +
                 0.025 * (m4 + m6) + mrt_V7 * m9 + mrt_V11 * m10 +
                 mrt_V8 * m11 + mrt_V12 * m12 + 0.25 * m13 +
                 0.125 * (m16 - m17);
            dist[7 * Np + n] = fq;
            ...
            fq = mrt_V1 * Den + mrt_V9 * m1 + mrt_V10 * m2 + 0.1 * (jy - jx) +
                 0.025 * (m6 - m4) + mrt_V7 * m9 + mrt_V11 * m10 +
                 mrt_V8 * m11 + mrt_V12 * m12 - 0.25 * m13 -
                 0.125 * (m16 + m17);
            dist[10 * Np + n] = fq;
            ...
            fq = mrt_V1 * Den + mrt_V9 * m1 + mrt_V10 * m2 + 0.1 * (jx + jz) +
                 0.025 * (m4 + m8) + mrt_V7 * m9 + mrt_V11 * m10 -
                 mrt_V8 * m11 - mrt_V12 * m12 + 0.25 * m15 +
                 0.125 * (m18 - m16);
            dist[11 * Np + n] = fq;
            ...
            fq = mrt_V1 * Den + mrt_V9 * m1 + mrt_V10 * m2 + 0.1 * (jz - jx) +
                 0.025 * (m8 - m4) + mrt_V7 * m9 + mrt_V11 * m10 -
                 mrt_V8 * m11 - mrt_V12 * m12 - 0.25 * m15 +
                 0.125 * (m16 + m18);
            dist[14 * Np + n] = fq;
...
__global__ void dvc_ScaLBL_D3Q19_AAodd_Greyscale_MRT(
    ...
    for (int s = 0; s < S; s++) {
        ...
        if (n < finish) {
            ...
            m1 = m1 + rlx_setA * ((19 * (ux * ux + uy * uy + uz * uz) * rho0 /
                                       porosity -
                                   11 * rho) -
                                  m1);
            m2 = m2 +
                 rlx_setA * ((3 * rho - 5.5 * (ux * ux + uy * uy + uz * uz) *
                                            rho0 / porosity) -
                             m2);
            jx = jx + Fx;
            ...
            m8 = m8 + rlx_setB * ((-0.6666666666666666 * uz * rho0) - m8) +
                 (1 - 0.5 * rlx_setB) * (-0.6666666666666666 * Fz);
            m9 = m9 + rlx_setA * (((2 * ux * ux - uy * uy - uz * uz) * rho0 /
                                   porosity) -
                                  m9);
            m10 = m10 + rlx_setA * (-m10);
            //m10 = m10 + rlx_setA*(-0.5*rho0*((2*ux*ux-uy*uy-uz*uz)/porosity)- m10);
            m11 = m11 +
                  rlx_setA * (((uy * uy - uz * uz) * rho0 / porosity) - m11);
...
__global__ void dvc_ScaLBL_D3Q19_GreyIMRT_Init(double *dist, int Np,
                                               double Den) {
    int n;
    int S = Np / NBLOCKS / NTHREADS + 1;
    for (int s = 0; s < S; s++) {
